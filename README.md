...
1. Check that you have your .env.dev and .env.dev.db.  If for some reason these files are not generated, remember that you can find samples in the repository.
   ...
   DEBUG=1
   SECRET_KEY=rijefdsuhjrkfeifdsksifksilfksifks
   DJANGO_ALLOWED_HOSTS=localhost 127.0.0.1 [::1]
   SQL_ENGINE=django.db.backends.postgresql
   SQL_DATABASE=patatasdb_dev
   SQL_USER=patatasdb
   SQL_PASSWORD=patataspass
   SQL_HOST=db
   SQL_PORT=5432
   DATABASE=postgres
   ...
   POSTGRES_USER=patatasdb
   POSTGRES_PASSWORD=patataspass
   POSTGRES_DB=patatasdb_dev
    ```

2. Create docker volumes to save the data:
    ```sh
    $ docker volume create --name=patatas_postgres_data_dev
    $ docker volume create --name=patatas_grafana
    ```
 ...
 * **Environment files:** Once the server is created, you must copy the .env files in the production server inside **/opt/patatas** path.
    ```sh
    $ cat .env.prod
    $ cat .env.prod.db
